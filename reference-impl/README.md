...
`review_launcher.py` 固定跑三个角色：`issues`（问题发现）、`simplification`（简化审查，删减建议无需举证）、`research`（最佳实践调研）。三路并发、互不可见，结果直接交主 Agent 裁决，**不设聚合位子代理，也不再提供对抗位**。每路产出一份 receipt（模型/档位钉参、prompt/output 的 sha256、session id、禁分叉核验结果），`finalize --mode parallel --verdict ...` 把三份 receipt 组装成一份不可变、内容寻址的 manifest；Verdict 必须由主 Agent 显式写入，不会从审查位输出里抽取。`parallel-run` 是这条路径的入口；旧的 `dual-run` 会直接拒绝并提示改用 `parallel-run`。

旧 dual-lens manifest（regular / adversarial / aggregate）不能用本版 `verify` 原地升级，这是协议换代，不是 drop-in。
...
4. **`review-templates/*.md`**：这三份角色合同已经是通用文本，可以直接用；想调整审查风格就改这里，脚本按脚本所在目录自动定位这三个文件，不依赖你把 `reference-impl/` 放在仓库的哪个位置。
...
以上修复都在一个全新临时仓库里走完整条 `new → dual-run → finalize → attempt → plan-approve → impl-start → post-approve → close → verify → render` 链路实测过（当时仍是 dual-lens 入口）。2026-08-19 起入口换成 `parallel-run`，状态机与 finalize 约束沿用上述实测；三路并发不再走对抗位/聚合位，Verdict 改由主 Agent 经 `--verdict` 写入。
...
| mode | `single-review` / `parallel-review` |
| source | 三路并行时问题主要来源：`issues` / `simplification` / `research` / `mixed`；单路填 `-` |
...
以下命令形状与当前协议一致。完整链路需要你自己的第二厂商 CLI；状态机字段语义见 `review_ledger.py` 里 `validate_manifest_data()` / `verify_state()`。
...
python3 reference-impl/review_launcher.py parallel-run --review-id 2026-01-01-example-change --phase pre \
  --user-request-file request.md --material-file plan.md > parallel.json
# parallel.json 是形如 {"issues": "...", "research": "...", "simplification": "..."} 的 JSON，三条都是 receipt 路径

ISSUES=$(python3 -c "import json;print(json.load(open('parallel.json'))['issues'])")
SIMPLIFICATION=$(python3 -c "import json;print(json.load(open('parallel.json'))['simplification'])")
RESEARCH=$(python3 -c "import json;print(json.load(open('parallel.json'))['research'])")

python3 reference-impl/review_launcher.py finalize --review-id 2026-01-01-example-change --phase pre --n 1 \
  --mode parallel --verdict approve \
  --receipt issues=$ISSUES --receipt simplification=$SIMPLIFICATION --receipt research=$RESEARCH \
...
# 只有主 Agent 裁决为 approve 时才能进这一步；user-confirm.md 内容需与上面 --user-quote-file 一致
