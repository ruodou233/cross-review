...
同一个模型写方案时，常会把自己的盲区也带进总结和自检里。`cross-review` 把重要方案、diff 或执行结果交给第二厂商模型做只读独立审查，强制保留用户原始需求、待审内容原文和固定开放两问。它适合在定稿前、重要修改落地后、或你想要第二意见时使用。核心原则是无副作用：审查方只读材料，只输出判断和问题，不替你修改工作区。

## 核心功能/亮点

- 固定”审查输入三要素”：用户原始需求、待审内容原文、固定开放两问。
- 要求结构化输出：`Verdict`、三分类 `Issues`、`Summary` 和两问必答。
...
- “我想要第二意见，看看有没有更优做法。”
  - Agent 会保留完整待审内容原文，让独立模型回答更优做法和冗余删改。
