...
> 安全 flag 与 `bin/codex-review-drain` 的 `call_codex` 完全一致（`--ephemeral --skip-git-repo-check -s read-only`），prompt 走 stdin（`-`）。模型档位显式指定为当前已验证并选定的默认档（作者环境 2026-07-20 起：三个正式执行角色 regular/adversarial/aggregate 默认 `low` 提速，按实测个别调档，不以三路耗时相近为统一调档目标——regular/adversarial 是并发两路，对齐它们能缩短批次墙钟；aggregate 是串行下游，只按归并质量调档，提高它只会增加总耗时。经命令行显式升级的上限为 `max`；含自动任务委派的档位（如 `ultra`）不用于审查位——自动集群破坏单代理深读语义与双 lens 独立性；指认过时由每日审计更新）。`codex` 建议经稳定入口 symlink（如 `~/.local/bin/codex`）调用，App 更名/迁移时只修 symlink；不在 PATH 时用全路径或设 `CODEX_BIN`。
