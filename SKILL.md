...
> 安全 flag 与 `bin/codex-review-drain` 的 `call_codex` 完全一致（`--ephemeral --skip-git-repo-check -s read-only`），prompt 走 stdin（`-`）。模型档位显式指定为当前 ChatGPT 订阅内最强档（2026-07-03 实测：`gpt-5.5-codex-max` 型号与 `max` 档均不可用，最强为 `gpt-5.5` + `xhigh`；指认过时由每日审计更新）。`codex` 二进制在本机为 `/Applications/Codex.app/Contents/Resources/codex`；不在 PATH 时用全路径或设 `CODEX_BIN`。
...
**致使用者**：有修改方案或问题，通过这些渠道能找到我——GitHub [ruodou233/cross-review](https://github.com/ruodou233/cross-review) 提 issue；小红书：错误乱码；微信公众号：能工智人错误乱码；B站：若逗道人。其他开源 skill 见 [GitHub 主页](https://github.com/ruodou233)。
